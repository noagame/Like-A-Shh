  ...
  return (
    <>
      <button
        ...
        className="w-full sm:w-auto bg-amber-500/15 border border-amber-400/40 text-amber-200 px-3.5 py-2.5 rounded-xl text-[11px] font-bold uppercase tracking-[0.16em] shadow-lg shadow-amber-500/5 transition-all hover:bg-amber-500/25 active:scale-95 text-center cursor-pointer"
      >
        + Agregar Evento
      </button>

      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/75 backdrop-blur-md p-3 sm:p-4 overflow-y-auto pt-12 pb-12">
          <div className="relative my-4 w-full max-w-xl rounded-2xl border border-white/10 bg-[#0d0d10] p-5 sm:p-7 shadow-[0_24px_80px_rgba(0,0,0,0.8)] backdrop-blur-2xl max-h-[90vh] overflow-y-auto text-white">
            <button
              type="button"
              onClick={() => setIsOpen(false)}
              className="absolute right-4 top-4 text-xl font-bold text-white/40 hover:text-white transition-colors"
            ...
            <form action={handleSubmit} className="space-y-4 text-xs sm:text-sm">
              ...
              <div>
                ...
                <textarea
                  name="description"
                  rows={3}
                  placeholder="Detalles de la sesión o evento..."
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    name="status"
                    defaultValue="draft"
                    className="w-full rounded-xl border border-white/10 bg-black/50 px-3 py-2.5 text-white focus:border-amber-400 focus:outline-none"
                  >
